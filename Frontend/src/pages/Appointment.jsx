...
const AppointmentCard = ({ appointment, startVideoCall }) => (
  <div className="flex items-center justify-between p-5 bg-white shadow-lg rounded-xl border border-gray-200">
    <div>
      <h3 className="text-lg font-semibold">
        Doctor: {appointment.appointmentWith || "Unknown"}
      </h3>
      <p className="text-sm text-gray-500">
        <b>Scheduled On:</b> {appointment.appointmentAt || "N/A"}
      </p>
      <p className="text-sm text-gray-600">
        <b>Time Slot:</b> {appointment.timeSlot || "N/A"}
      </p>
    </div>
  ...
  useEffect(() => {
    const fetchAppointment = async () => {
      try {
        ...
        console.log("Fetching appointment for email:", email);
        ...
        if (data.status === "fetched") {
          ...
          setAppointment(null); // No appointment found
        }
      } catch (error) {
        console.error("Error fetching appointment:", error);
        setAppointment(null);
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500">
          No booked appointment available.
        </p>
      ) : (
        <AppointmentCard
