...
const DoctorLogin = () => {
  const { handleSetUser } = useAuth(); // Get authentication functions
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const navigate = useNavigate();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.status === "Doctor found") {
        console.log("Doctor Found:", res.data.user);
        
        // Set user role as "doctor" in context
        handleSetUser({ name: res.data.user.name, email, role: "doctor" });

        // Redirect to Doctor Dashboard with doctor data
